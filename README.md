...
* This second table includes hyperparameters that are not essential but can have a significant impact on algorithm performance. 
  ...
  * The *desired_bin_effect* hyperparameter preserves the raw default behavior, requires protective bins where the *> threshold* group has better censoring-aware survival, requires high-risk bins where that group has worse survival, or uses the alternate permissive binary encoding.
