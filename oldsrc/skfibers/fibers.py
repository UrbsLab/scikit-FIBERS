...
class FIBERS(BaseEstimator, TransformerMixin):
    def __init__(self, label_name="Class", duration_name="Duration",
                 ...
                 mutation_strategy="Regular", random_seed=None, threshold=0, evolving_probability=1,
    ...
    def get_experiment_output(self, X, y=None):
        l = list()
        if not self.hasTrained:
            raise Exception("Model must be trained first")
        # Ordering the bin scores from best to worst
        durations_no, durations_mm, event_observed_no, event_observed_mm, top_bin = self.get_duration_event(0)
        results = logrank_test(durations_no, durations_mm, event_observed_A=event_observed_no,
                               event_observed_B=event_observed_mm)
        ...
        sorted_bin_scores = dict(sorted(self.bin_scores.items(), key=lambda item: item[1], reverse=True))
        sorted_bin_list = list(sorted_bin_scores.keys())
        Bin = self.bins[sorted_bin_list[0]]
        d_data = X.copy()
        ...
        coxmodeldata =  d_data[["Bin", self.duration_name, self.label_name]]
        cph = CoxPHFitter()
        cph.fit(coxmodeldata, self.duration_name, event_col=self.label_name, show_progress=True)
        summary = cph.summary
        ...
        pdf = pd.DataFrame([[self.bins[top_bin], str(self.bins[top_bin]).count('P'), str(self.bins[top_bin]).count('R'), 
                             len(self.bins[top_bin]), str(self.bins[top_bin]).count('P')/len(self.bins[top_bin]), 
                             None if str(self.bins[top_bin]).count('P') != 10 else 101,
                             self.bin_scores[top_bin], 
                             HR, HR_CI, HR_p_value, self.elapsed_time,
                             len(durations_no),
                             len(durations_mm), len(durations_no)/len(durations_mm), 
                             results.p_value, self.bins[top_bin].get_threshold(), self.score(X, y) if y is not None else None]],
                           columns=columns).T  # SPHIA
        return pdf
