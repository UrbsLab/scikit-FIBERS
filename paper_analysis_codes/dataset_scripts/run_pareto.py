...
class RunFIBERS:
    def __init__(self):
        #Run Parameters
        self.write_path = '/project/kamoun_shared/output_shared/bandheyh/FIBERS-summer/pareto/'
        self.data_path = '/project/kamoun_shared/data_shared/FIBERS-summer/pareto/'
        ...
        self.survivability_feature = 1
        self.threshold = 0
        self.survivability_threshold = 0
        self.survivability_benefit = (0.1, 0.01)
        ...
        if self.run_cluster == 'LSF':
            submit_lsf_cluster_job(self,self.instance,self.pred_feature,self.nc,self.noise,self.total_feature,self.threshold,
                                   self.censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
            jobCount +=1
        elif self.run_cluster == 'SLURM':
            submit_slurm_cluster_job(self,self.instance,self.pred_feature,self.nc,self.noise,self.total_feature,self.threshold,
                                     self.censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
            jobCount +=1
        else:
        ...
        exp_name = 'BaseNC'
        if self.run_cluster == 'LSF':
            submit_lsf_cluster_job(self,self.instance,self.pred_feature,'True',self.noise,self.total_feature,self.threshold,self.censor,
                                   self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
            jobCount +=1
        elif self.run_cluster == 'SLURM':
            submit_slurm_cluster_job(self,self.instance,self.pred_feature,'True',self.noise,self.total_feature,self.threshold,self.censor,
                                     self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
            jobCount +=1
        else:
            print('ERROR: Cluster type not found')
        ...
        for censor in self.censoring:
            if self.run_cluster == 'LSF':
                submit_lsf_cluster_job(self,self.instance,self.pred_feature,self.nc,self.noise,self.total_feature,self.threshold,censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
                jobCount +=1
            elif self.run_cluster == 'SLURM':
                submit_slurm_cluster_job(self,self.instance,self.pred_feature,self.nc,self.noise,self.total_feature,self.threshold,censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
                jobCount +=1
            else:
                print('ERROR: Cluster type not found')
        ...
        for instance in self.instances:
            if self.run_cluster == 'LSF':
                submit_lsf_cluster_job(self,instance,self.pred_feature,self.nc,self.noise,self.total_feature,self.threshold,self.censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
                jobCount +=1
            elif self.run_cluster == 'SLURM':
                submit_slurm_cluster_job(self,instance,self.pred_feature,self.nc,self.noise,self.total_feature,self.threshold,self.censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
                jobCount +=1
            else:
                print('ERROR: Cluster type not found')
        ...
        for noise in self.noises:
            if self.run_cluster == 'LSF':
                submit_lsf_cluster_job(self,self.instance,self.pred_feature,self.nc,noise,self.total_feature,self.threshold,self.censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
                jobCount +=1
            elif self.run_cluster == 'SLURM':
                submit_slurm_cluster_job(self,self.instance,self.pred_feature,self.nc,noise,self.total_feature,self.threshold,self.censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
                jobCount +=1
            else:
                print('ERROR: Cluster type not found')
        ...
        for total_feature in self.total_features:
            if self.run_cluster == 'LSF':
                submit_lsf_cluster_job(self,self.instance,self.pred_feature,self.nc,self.noise,total_feature,self.threshold,self.censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
                jobCount +=1
            elif self.run_cluster == 'SLURM':
                submit_slurm_cluster_job(self,self.instance,self.pred_feature,self.nc,self.noise,total_feature,self.threshold,self.censor,self.survivability_feature,self.survivability_threshold,self.survivability_benefit[0],self.survivability_benefit[1],exp_name)
                jobCount +=1
            else:
                print('ERROR: Cluster type not found')
...
def submit_slurm_cluster_job(self,instance,pred_feature,nc,noise,total_feature,threshold,censor,sf,st,sbm,sbs,exp_name): #legacy mode just for cedars (no head node) note cedars has a different hpc - we'd need to write a method for (this is the more recent one)
    ...
    job_name = 'FIBERS_data_sim_'+'i_'+str(instance)+'_tf_'+str(total_feature)+'_p_'+str(pred_feature)+'_sf_'+str(sf)+'_st_'+str(st)+'_sbm_'+str(sbm)+'_sbs_'+str(sbs)+'_t_'+str(threshold)+'_n_'+str(noise)+'_c_'+str(censor)+'_nc_'+str(nc)+'_'+job_ref
    ...
    sh_file.write('srun python job_pareto.py' +' --o '+self.data_path+' --i '+str(instance)+' --p '+ str(pred_feature) +' --nc '+str(nc) +' --n '+str(noise) +' --tf '+str(total_feature)+' --t '+str(threshold)+' --c '+str(censor)+
                  ' --sf '+str(sf)+' --st '+ str(st) +' --sbm '+str(sbm) + ' --sbs '+str(sbs)+
                  ' --l '+str(exp_name)+ '\n')
    sh_file.close()
    os.system('sbatch ' + job_path)


def submit_lsf_cluster_job(self,instance,pred_feature,nc,noise,total_feature,threshold,censor,sf,st,sbm,sbs,exp_name): #UPENN - Legacy mode (using shell file) - memory on head node
    job_ref = str(time.time())
    job_name = 'FIBERS_data_sim_'+'i_'+str(instance)+'_tf_'+str(total_feature)+'_p_'+str(pred_feature)+'_sf_'+str(sf)+'_st_'+str(st)+'_sbm_'+str(sbm)+'_sbs_'+str(sbs)+'_t_'+str(threshold)+'_n_'+str(noise)+'_c_'+str(censor)+'_nc_'+str(nc)+'_'+job_ref
    job_path = self.scratchPath+'/'+job_name+ '_run.sh'
    sh_file = open(job_path, 'w')
    ...
    sh_file.write('python job_pareto.py'+' --o '+self.data_path+' --i '+str(instance)+' --p '+ str(pred_feature) +' --nc '+str(nc) +' --n '+str(noise) +' --tf '+str(total_feature)+' --t '+str(threshold)+' --c '+str(censor)+
                  ' --sf '+str(sf)+' --st '+ str(st) +' --sbm '+str(sbm) + ' --sbs '+str(sbs)+
                  ' --l '+str(exp_name)+ '\n')
    sh_file.close()
