...
class RunFIBERS:
    def __init__(self):
        ...
        self.data_path = '/project/kamoun_shared/data_shared/simulation_study_covariates_simple/'
...
def submit_slurm_cluster_job(self,instance,pred_feature,nc,noise,total_feature,threshold,censor,exp_name): #legacy mode just for cedars (no head node) note cedars has a different hpc - we'd need to write a method for (this is the more recent one)
    ...
    job_name = 'FIBERS_data_sim_simple_'+'i_'+str(instance)+'_tf_'+str(total_feature)+'_p_'+str(pred_feature)+'_t_'+str(threshold)+'_n_'+str(noise)+'_c_'+str(censor)+'_nc_'+str(nc)+'_'+job_ref
    ...
    sh_file.write('srun python job_covariate_simple_sim.py' +' --o '+self.data_path+' --i '+str(instance)+' --p '+ str(pred_feature) + ' --tf '+str(total_feature) + ' --l '+str(exp_name)+ '\n')
    sh_file.close()
...
def submit_lsf_cluster_job(self,instance,pred_feature,nc,noise,total_feature,threshold,censor,exp_name): #UPENN - Legacy mode (using shell file) - memory on head node
    ...
    sh_file.write('python job_covariate_simple_sim.py'+' --o '+self.data_path+' --i '+str(instance)+' --p '+ str(pred_feature) +' --tf '+str(total_feature)+' --l '+str(exp_name)+ '\n')
    sh_file.close()
