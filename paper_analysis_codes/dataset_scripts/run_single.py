...
class RunFIBERS:
    def __init__(self):
        #Run Parameters
        self.write_path = '/project/kamoun_shared/output_shared/bandheyh/FIBERS-summer/single/'
        self.data_path = '/project/kamoun_shared/data_shared/FIBERS-summer/single/'
...
def submit_lsf_cluster_job(self,instance,pred_feature,nc,noise,total_feature,threshold,censor,exp_name): #UPENN - Legacy mode (using shell file) - memory on head node
    ...
    sh_file.write('python job_single.py'+' --o '+self.data_path+' --i '+str(instance)+' --p '+ str(pred_feature) +' --nc '+str(nc) +' --n '+str(noise) +' --tf '+str(total_feature)+' --t '+str(threshold)+' --c '+str(censor)+' --l '+str(exp_name)+ '\n')
    sh_file.close()
