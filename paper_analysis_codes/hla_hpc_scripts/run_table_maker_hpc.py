...
def main(argv):
    ...
    if run_cluster == 'LSF':
        submit_lsf_cluster_job(scratchPath,logPath,writepath,outputpath,random_seeds,reserved_memory,queue, replicates)
    elif run_cluster == 'SLURM':
        submit_slurm_cluster_job(scratchPath,logPath,writepath,outputpath,random_seeds,reserved_memory,queue, replicates)
    else:
...
def submit_slurm_cluster_job(scratchPath,logPath,writepath,outputpath,random_seeds,reserved_memory,queue, replicates): #legacy mode just for cedars (no head node) note cedars has a different hpc - we'd need to write a method for (this is the more recent one)
    job_ref = str(time.time())
    job_name = 'Table_FIBERS_' +'REAL'+'_'+job_ref
    ...
    sh_file.write('srun python job_table_maker_hpc.py'+' --w '+writepath+' --o '+outputpath +' --rs '+ str(random_seeds)+' --re '+ str(replicates) + '\n')
    sh_file.close()
    os.system('sbatch ' + job_path)


def submit_lsf_cluster_job(scratchPath,logPath,writepath,outputpath,random_seeds,reserved_memory,queue, replicates): #UPENN - Legacy mode (using shell file) - memory on head node
    job_ref = str(time.time())
    job_name = 'Table_FIBERS_' +'REAL'+'_'+job_ref
    job_path = scratchPath+'/'+job_name+ '_run.sh'
    sh_file = open(job_path, 'w')
    ...
    sh_file.write('python job_table_maker_hpc.py'+' --w '+writepath +' --o '+outputpath +' --rs '+ str(random_seeds)+' --re '+ str(replicates) + '\n')
    sh_file.close()
