...
def main(argv):
    ...
    load_and_split_dataset(datafile,outputfolder,partitions)

# Function to save the datasets to CSV files
def save_fold(train_data, test_data, fold_idx, output_folder,file_name):
...
# Load the dataset
def load_and_split_dataset(csv_file,output_folder,partitions):
    ...
    for fold_idx, (train_index, test_index) in enumerate(kf.split(data), start=1):
        ...
        save_fold(train_data, test_data, fold_idx, output_folder,file_name)
