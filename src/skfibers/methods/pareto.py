...
class PARETO:
    def __init__(self):  
        ...
        self.max_obj_1 = None #max_score
        self.max_obj_2 = None #max_area
        self.min_obj_2 = None
    ...
    def update_front(self,candidate_metric_1,candidate_metric_2,objectives):
        ...
        if not candidate_bin in self.bin_front: # Check that candidate rule objectives are not equal to any existing objective pairs on the front
            ...
            self.max_obj_1 = max(self.bin_front, key=lambda x: x[0])[0]
            self.max_obj_2 = max(self.bin_front, key=lambda x: x[1])[1]
            self.min_obj_2 = min(self.bin_front, key=lambda x: x[1])[1]
            # if max score and area have been found, scale the bin
            if self.max_obj_1 != None and self.max_obj_2 != None:
                self.bin_front_scaled = [(x[0] / float(self.max_obj_1), x[1] / float(self.max_obj_2)) for x in self.bin_front]
            else:
    ...
    def delete_from_front(self, candidate_metric_1, candidate_metric_2):
        ...
        bin_scaled = (candidate_metric_1/self.max_obj_1, candidate_metric_2/self.max_obj_2)
        self.bin_front.remove(bin)
        self.bin_front_scaled.remove(bin_scaled)

        # update max score and area in case a max bin was deleted
        if candidate_metric_1 == self.max_obj_2 or candidate_metric_2 == self.max_obj_1:
            self.max_obj_1 = max(self.bin_front, key=lambda x: x[0])[0]
            self.max_obj_2 = max(self.bin_front, key=lambda x: x[1])[1]
            self.min_obj_2 = min(self.bin_front, key=lambda x: x[1])[1]
    ...
    def dominates(self,p,q,objectives):
        ...
        # Ensure replacement only if log rank score is higher when sizes are equal
    ...
    def get_pareto_fitness(self,obj_1,obj_2, landscape):
        scaled_obj_1 = obj_1 / self.max_obj_1
        scaled_obj_2 = obj_2 / self.max_obj_2
        ...
        if landscape: # Handles special cases when calculating fitness landscape for visualization
            if obj_1 > self.max_obj_1 or obj_2 < self.min_obj_2:
                return 1.0
            if self.point_beyond_front(obj_1, obj_2):
                return 1.0 
        if obj_1 == 0.0 and obj_2 == self.max_obj_2:
            return 0.0
        elif (obj_1,obj_2) in self.bin_front: # Rules on the front return an ideal fitness
            return 1.0
        elif obj_1 > self.max_obj_1:
            return 1.0
        elif obj_2 < self.min_obj_2:
            return 1.0
        #elif rule_coverage == self.metric_limits[1]: #rule has the maximum value of one objective
        #    return 1.0
        else:
            bin_objectives = (scaled_obj_1, scaled_obj_2)
            # Find the closest distance between the bin and the pareto front
            temp_front = [] #max area boundary
            for front_point in self.bin_front_scaled:
                temp_front.append(front_point)
            temp_front.append([self.max_obj_1, self.max_obj_2]) #max log rank score boundary
    ...
    def point_beyond_front(self,obj_1,obj_2):
    ...
    def plot_zoomed_pareto_landscape(self, resolution, min_area, bin_pop, show, save, output_path, data_name):
        ...
        x = np.linspace(0.995 * min_area, 1.005 * self.max_obj_2,resolution) #area
        y = np.linspace(0,1.025 * self.max_obj_1,resolution) #log rank
        #X,Y = np.meshgrid(x,y)
        ...
        im = plt.imshow(Z, extent=[0.995 * min_area, 1.005*self.max_obj_2, 0, 1.025*self.max_obj_1], interpolation='nearest', origin='lower', cmap='magma', aspect='auto') #cmap='viridis' 'magma', alpha=0.6
        ...
        plt.xlim(0.995 * min_area, 1.005*self.max_obj_2)
        plt.ylim(0, 1.025*self.max_obj_1)
        custom_xticks = np.around(np.linspace(0.995 * min_area, 1.005*self.max_obj_2, 10), 3)
        custom_xlabels = np.around(np.linspace(0.995 * min_area, 1.005*self.max_obj_2, 10), 3)
        plt.xticks(custom_xticks, custom_xlabels)

        custom_yticks = np.around(np.linspace(0, 1.025*self.max_obj_1, 10), 1)
        custom_ylabels = np.around(np.linspace(0, 1.025*self.max_obj_1, 10), 1)
    ...
    def plot_pareto_landscape(self, resolution, min_area, bin_pop, show, save, output_path, data_name):
        # Generate fitness landscape ******************************
        x = np.linspace(0, self.max_obj_2 + 1,resolution) #bin size
        y = np.linspace(0, 1.025 * self.max_obj_1,resolution) #log rank
        #X,Y = np.meshgrid(x,y)
        Z = [[None for _ in range(resolution)] for _ in range(resolution)]
        for i in range(len(x)):
        ...
        # Plot Setup *********************************************
        plt.figure(figsize=(10,6)) #(10, 8))
        im = plt.imshow(Z, extent=[0, self.max_obj_2 + 1, 0, 1.025*self.max_obj_1], interpolation='nearest', origin='lower', cmap='magma', aspect='auto') #cmap='viridis' 'magma', alpha=0.6
        # Add colorbar for the gradient
        cbar = plt.colorbar(im)
        cbar.set_label('Fitness Value')
        ...
        plt.plot(np.array(metric_2_front_list), np.array(metric_1_front_list), 'o-', ms=10, lw=2, color='black')
        # Plot pareto front boundaries to plot edge
        plt.plot([metric_2_front_list[-1], self.max_obj_2 + 1], [metric_1_front_list[-1], metric_1_front_list[-1]], '--', lw=1, color='black')
        plt.plot([metric_2_front_list[0],metric_2_front_list[0]],[metric_1_front_list[0],0],'--',lw=1, color='black') # Accuracy line
        # Add labels and title
        plt.xlabel('Bin Size', fontsize=14)
        plt.ylabel('Log Rank Score', fontsize=14)
        # Set the axis limits between 0 and 1
        plt.xlim(0, self.max_obj_2 + 1)
        plt.ylim(0, 1.025*self.max_obj_1)
        custom_xticks = list(range(0, self.max_obj_2 + 1))
        custom_xlabels = list(range(0, self.max_obj_2 + 1))
        plt.xticks(custom_xticks, custom_xlabels)
        ...
        for i in range(len(bin_pop)):
            bin = bin_pop[i]
            master_metric_1_list.append(bin.log_rank_score)
            master_metric_2_list.append(bin.bin_size)
