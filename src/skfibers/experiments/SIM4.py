...
""" SIM4 is a custom simulator developed by Brian Ling with help from Keith McCoulough designed to create more realistic covariate effects
    based on real worl data observations. It is most closely based on SIM3. """
...
def survival_data_simulation_categorical_covariates(instances=10000,total_features=100,predictive_features=5,feature_frequency_range=(0.1, 0.4),noise_frequency=0.0,censoring_frequency=0.2,negative_control=False,random_seed=None, threshold=None):
    ...
    for i in range(0, instances):
        ...
        african_american_coef = np.log(1.23)
        hispanic_coef = np.log(0.77)
        asian_coef = np.log(0.7)
        other_coef = np.log(1.00)
    ...
    # Next 4 lines are for dropping a covariate column to prevent convergence issues
    df_race = pd.get_dummies(race_list, dtype=int)
